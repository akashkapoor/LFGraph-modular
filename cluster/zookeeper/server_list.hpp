...
#ifndef LFGRAPH_ZOOKEEPER_SERVER_LIST_HPP
#define LFGRAPH_ZOOKEEPER_SERVER_LIST_HPP
...
private:
  ...
  lfgraph::recursive_mutex watchlock;
 ...
 *  and joining the service through the use of callbacks.
 */
class zorro_server_list {
public:
  
  ///  Joins a zookeeper cluster.
  ///  Zookeeper nodes will be created in the prefix "prefix".
  ///  The current machine will be identified as "serveridentifier"
  zorro_server_list(std::vector<std::string> zkhosts,
                    std::string prefix,
                    std::string serveridentifier,
                    std::string state);
  ...
  /// gets a list of all servers in a namespace
  std::vector< std::pair<size_t, std::string> > get_all_servers(std::string name_space);
  
  /// Watches for changes to a namespace while returning the current contents
  /// When changes occur, the callback is called.
  std::vector< std::pair<size_t, std::string> > watch_changes(std::string name_space);
  
  /// Removes the watch callback.
  void stop_watching(std::string name_space);
  
  /// Unsets the replacement flag for the server once it becomes a part of
  /// the cluster.
  void unset_replacement_flag();
   ...
   * the callback. Note that the callback may be triggered in a different thread.
   */
  void set_callback(boost::function<void(zorro_server_list* cur,
                                         std::string name_space,
...
private:
  ...
  boost::function<void(zorro_server_list*, std::string, std::vector< std::pair<size_t, std::string> >)> callback;
