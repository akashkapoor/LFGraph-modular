...
zorro_server_list::zorro_server_list(std::vector<std::string> zkhosts,
                                     std::string _prefix,
                                     std::string _serveridentifier,
                                     std::string _state) :
prefix(_prefix), serveridentifier(_serveridentifier), callback(NULL), state(_state) {
  // construct hosts list
  std::string hosts = boost::algorithm::join(zkhosts, ",");
  prefix = normalize_path(prefix);
  if (prefix[0] != '/') prefix = "/" + prefix;
...
zorro_server_list::~zorro_server_list() {
  if (handle != NULL) zookeeper_close(handle);
}


std::vector< std::pair<size_t, std::string> > zorro_server_list::get_all_servers(std::string name_space) {
  boost::algorithm::trim(name_space); assert(name_space.length() > 0);
  struct String_vector children;
  children.count = 0;
  children.data = NULL;
  ...
  // if there are no children quit
  if (stat == ZNONODE) return ret;
  ret = String_vector_to_pair_vector_int(&children);
  free_String_vector(&children);
  return ret;
}

/// Joins a namespace
void zorro_server_list::join(std::string name_space) {
  //logstream(LOG_INFO) << "NS: " << name_space << std::endl;
  boost::algorithm::trim(name_space); assert(name_space.length() > 0);
  //logstream(LOG_INFO) << "PRefix: " << prefix << std::endl;
  //logstream(LOG_INFO) << "Server id: " << serveridentifier << std::endl;
  create_dir(handle,
             prefix + name_space,
             "zk_server_list");
  std::string path = normalize_path(prefix + name_space) + serveridentifier;
  ...
  //logstream(LOG_INFO) << "Path in join: " << path << std::endl;
  int stat = create_ephemeral_node(handle, path, "");
  if (stat == ZNODEEXISTS) {
    std::cerr << "Server " << serveridentifier << " already exists!" << std::endl;
  }
...
void zorro_server_list::leave(std::string name_space) {
  boost::algorithm::trim(name_space); assert(name_space.length() > 0);
  std::string path = normalize_path(prefix + name_space) + serveridentifier;
  //path += (replacement ? "1" : "0");
  path = path + "," + state;
  //std::cout << "PATH: " << path << std::endl;
  //logstream(LOG_INFO) << "Path in leave: " << path << std::endl;
  delete_node(handle, path, "zk_server_list leave");
  // also try to delete its parents if they become empty
  delete_dir(handle, prefix + name_space, "zk_server_list leave cleanup");
  if (prefix != "/") delete_dir(handle,
...
std::vector< std::pair <size_t, std::string> > zorro_server_list::watch_changes(std::string name_space) {
  boost::algorithm::trim(name_space); assert(name_space.length() > 0);
  struct String_vector children;
  children.count = 0;
  children.data = NULL;
...
void zorro_server_list::stop_watching(std::string name_space) {
  boost::algorithm::trim(name_space); assert(name_space.length() > 0);
  std::string path = prefix + name_space;
  watchlock.lock();
  watches.erase(path);
...
void zorro_server_list::set_callback(boost::function<void(zorro_server_list*,
                                                          std::string name_space,
                                                          std::vector< std::pair <size_t, std::string> > server)
                                     > fn) {
  watchlock.lock();
  callback = fn;
  watchlock.unlock();
...
void zorro_server_list::issue_callback(std::string path) {
  watchlock.lock();
  // search for the path in the watch set
  bool found = watches.count(path);
  if (found) {
...
void zorro_server_list::watcher(zhandle_t *zh,
                                int type,
                                int state,
                                const char *path,
                                void *watcherCtx) {
  zorro_server_list* slist = reinterpret_cast<zorro_server_list*>(watcherCtx);
  if (type == ZOO_CHILD_EVENT) {
    std::string strpath = path;
    slist->issue_callback(path);
  }
